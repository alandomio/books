...
def export_pdf(chapter_files):
    ...
    # Add metadata
    cmd.extend(["--metadata", "title=La Grande Sbronza"])
...
def export_epub(chapter_files):
    ...
    cmd.extend(["--metadata", f"title=La Grande Sbronza"])
    cmd.extend(["--metadata", "lang=it-IT"])
