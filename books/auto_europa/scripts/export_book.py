...
def export_pdf(chapter_files):
    ...
    # Add metadata
    cmd.extend(["--metadata", "title=Suicidio su Commissione: L'Invasione Elettrica e il Declino Europeo"])
    cmd.extend(["--metadata", "author=Alan Domio"])
...
if __name__ == "__main__":
    ...
    # Also write a concatenated markdown file for easy reading
    output_md = os.path.join(PROJECT_ROOT, "book_complete.md")
    with open(output_md, "w", encoding="utf-8") as full_book:
        full_book.write("# Suicidio su Commissione: L'Invasione Elettrica e il Declino Europeo\n\n")
        full_book.write("## Di Alan Domio | Ralph (L'Ingegnere Disilluso)\n\n---\n\n")
        
        for ch in files:
            if os.path.exists(ch):
                with open(ch, "r", encoding="utf-8") as f:
                    content = f.read()
                    full_book.write(content)
                    full_book.write("\n\n---\n\n")
    print(f"Concatenated book to {output_md}")
