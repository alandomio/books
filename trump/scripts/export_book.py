...
def export_pdf(chapter_files):
    ...
    # Add metadata
    cmd.extend(["--metadata", "title=Year One: Cronache di una Repubblica che Si Smonta"])
...
def export_epub(chapter_files):
    ...
    cmd.extend(["--metadata", f"title=Year One: Cronache di una Repubblica che Si Smonta"])
    cmd.extend(["--metadata", "lang=it-IT"])
