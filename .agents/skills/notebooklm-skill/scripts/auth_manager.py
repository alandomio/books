...
class AuthManager:
    ...
    def is_authenticated(self) -> bool:
        ...
        age_days = (
            (time.time() - self.state_file.stat().st_mtime) / 86400
        )
        if age_days > 7:
            print(
                f"⚠️ Browser state is {age_days:.1f} days old, "
    ...
    def setup_auth(
        ...
        try:
            ...
            # Use the default page (avoids leftover about:blank tab)
            page = (
                context.pages[0]
                if context.pages
            ...
            print(
                f"  ⏱️  Waiting up to {timeout_minutes} minutes "
                "for login..."
            )
    ...
    def _save_browser_state(self, context: BrowserContext):
        ...
        try:
            ...
            print(
                f"  💾 Saved browser state to: {self.state_file}"
            )
        except Exception as e:
    ...
    def _save_auth_info(self):
        ...
        try:
            info = {
                ...
                'authenticated_at_iso': time.strftime(
                    '%Y-%m-%d %H:%M:%S'
                ),
            }
...
def main():
    ...
    setup_parser = subparsers.add_parser(
        'setup', help='Setup authentication'
    )
    ...
    subparsers.add_parser('validate', help='Validate auth')
    subparsers.add_parser('clear', help='Clear auth')
    ...
    if args.command == 'setup':
        ...
        if success:
            ...
            print("You can now use ask_question.py to query "
                  "NotebookLM")
        else:
        ...
        if info.get('state_age_hours'):
            hours = info['state_age_hours']
            print(f"  State age: {hours:.1f} hours")
